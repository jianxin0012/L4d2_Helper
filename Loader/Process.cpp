...
bool CProcess::Inject()
{
	...
	printf("%p", pLoadLibrary);
	if (!pLoadLibrary)
		return false;

	char shellCode[5] = {0x8B,0xFF,0x55,0x8B,0xEC};
	if (!WriteProcessMemory(m_hProcessHandle, reinterpret_cast<PVOID>(pLoadLibrary), shellCode, 5, 0))
		return false;
