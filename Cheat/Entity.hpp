...
public:
	...
	{
		constexpr std::uint32_t m_Boss[] = { 277, 276 };
		constexpr std::uint32_t m_Special[] = {0,99, 270, 263, 265, 272  };
		constexpr std::uint32_t m_Infected = 264;
		constexpr std::uint32_t m_Player[] = { 231, 275 };
	...
	inline EHANDLE& m_hActiveWeapon() {
		return *reinterpret_cast<EHANDLE*>(reinterpret_cast<unsigned long>(this) + g_Offsets.m_hActiveWeapon);
	};

	inline C_BaseCombatWeapon* GetActiveWeapon() {
		HANDLE handle = m_hActiveWeapon();
		return reinterpret_cast<C_BaseCombatWeapon*>(handle);
	}

	inline HANDLE WeaponHandle()
	{
		return *reinterpret_cast<HANDLE*>(reinterpret_cast<std::uintptr_t>(this) + g_Offsets.m_hActiveWeapon);
	}
};
