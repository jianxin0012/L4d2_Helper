...
void CESP::Execute()
{
	...
	for (std::uint32_t nIndex = 1; nIndex < g_Interfaces->EntityList->GetHighestEntityIndex(); nIndex++)
	{
		...
		char* name = pEntity->GetClientClass()->m_pNetworkName;


		if (pEntity->Team() == 3 && pEntity->GetGroup() && pEntity->ValidEntity())
			DrawPlayer( pEntity );
...
void CESP::DrawWatermark()
{
	...
	if (!bGotWatermark)
	{
		if (!MultiByteToWideChar( CP_UTF8, 0, "Cheat by X", -1, szWatermark, 128 ) > 0)
			return;
...
void CESP::DrawPlayer( CBaseEntity * pEntity )
{
	...
	else if (pEntity->GetGroup() == GROUP_SPECIAL || pEntity->GetGroup() == GROUP_BOSS)
	{
		char buffer[128];
		wchar_t wbuf[128];
...
void CESP::DrawBox( const RECT& rect, CBaseEntity* pEntity)
{
	if (pEntity->Team() == 3 && (pEntity->GetGroup() == GROUP_SPECIAL || pEntity->GetGroup() == GROUP_BOSS))
	{
		g_Interfaces->Surface->DrawSetColor(255, 0, 0, 255);
